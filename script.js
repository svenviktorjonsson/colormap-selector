class ColorEditor {
    ...
    setupCanvases() {
        ...
        size = Math.floor(size / checkerboardBaseSize) * checkerboardBaseSize;
    ...
    getActivePoint() {
        if (!this.state.points || this.state.points.length === 0) {
            // This is a safety net for unexpected empty states, though the new constructor prevents this.
            return { id: 0, hsPos: { u: 0, v: 0 }, originalHsPos: { u: 0, v: 0 }, brightness: 0.5, alpha: 1.0 };
        }
        return this.state.points.find(p => p.id === this.state.activePointId) || this.state.points[0];
    }
    ...
    drawHSSlice() {
        ...
        const { brightness, alpha } = activePoint;
        ...
        this.updateValidPointsCache(width, height, brightness);
    }
    ...
    // This function no longer deletes and recreates canvases
    renderNodes() {
        this.drawHSElements();
    ...
    drawHSElements() {
        ...
        this.state.points.forEach(point => {
            ...
            const isOnCurrentPlane = Math.abs(point.brightness - activePoint.brightness) < 0.01;
            ctx.save();
            ...
            ctx.fill();
            if (isOnCurrentPlane) {
                ctx.strokeStyle = point.id === this.state.activePointId ? '#3b82f6' : 'white';
                ctx.lineWidth = point.id === this.state.activePointId ? 3 : 2;
                ctx.stroke();
            } else {
                ctx.setLineDash([4, 4]);
    ...
    drawBrightnessElements() {
        ...
        ctx.clearRect(0, 0, canvas.width, canvas.height);
        const activePoint = this.getActivePoint();
        this.drawHorizontalLine(ctx, (1 - activePoint.brightness) * canvas.height);
        if (this.state.points.length > 1) {
        ...
        this.state.points.forEach((point, index) => {
            ...
            const color = this.abstractToRGB(point.hsPos.u, point.hsPos.v, point.brightness);
            const {r, g, b} = this.clampColor(color);
            this.drawPoint(ctx, x, y, r, g, b, point.id === this.state.activePointId);
        });
    }
    ...
    drawAlphaElements() {
        ...
        ctx.clearRect(0, 0, canvas.width, canvas.height);
        const activePoint = this.getActivePoint();
        this.drawHorizontalLine(ctx, (1 - activePoint.alpha) * canvas.height);
        if (this.state.points.length > 1) {
    ...
    drawPoint(ctx, x, y, r, g, b, isActive) {
        ctx.beginPath();
        ctx.arc(x, y, 9, 0, 2 * Math.PI);
        ctx.fillStyle = `rgb(${r}, ${g}, ${b})`;
        ctx.fill();
        ctx.strokeStyle = isActive ? '#3b82f6' : 'white';
    ...
    updateUIReadouts() {
        const activePoint = this.getActivePoint();
        const { brightness, alpha, hsPos } = activePoint;
    ...
    drawCheckerboard(ctx) {
        ...
        const squareSize = 8; // Use the fixed base size
        const numSquaresW = width / squareSize;
        const numSquaresH = height / squareSize;
    ...
    findClosestValidPoint(targetX, targetY) {
        // Return the center of the shape if the cache of valid points isn't ready.
        ...
        // First, quickly check if the target is already on a valid pixel.
        ...
        // Define a vector from the center of the shape to the cursor's position.
        const vecX = targetX - centerX;
        const vecY = targetY - centerY;
        const dist = Math.sqrt(vecX * vecX + vecY * vecY);

        if (dist < 1) { // If the target is at the center, return it.
            return { x: centerX, y: centerY };
        }

        // Search backwards along the vector from the cursor towards the center.
        // The first valid point we hit is the one on the shape's edge.
        const steps = Math.ceil(dist); // Use 1-pixel steps for precision.
        for (let i = steps; i >= 0; i--) {
            ...
            if (this.validPointsCache.has(`${checkX},${checkY}`)) {
                return { x: checkX, y: checkY }; // Return the edge point.
            }
        }

        // As a fallback, if no valid point is found (highly unlikely), return the center.
        return { x: centerX, y: centerY };
    }
    ...
    handleHSPointDrag(point, x, y) {
        ...
        point.originalHsPos = { u: au, v: av }; // Keep track of the actual cursor position.
        const { r, g, b } = this.abstractToRGB(au, av, point.brightness);

        if (this.isValidColor(r, g, b)) {
            // If the cursor is inside the valid area, the point follows it.
            point.hsPos = { u: au, v: av };
        } else {
            // If the cursor is outside, find the closest valid point on the edge.
    ...
    handleMouseDown(e, type) {
        ...
        if (hitPoint) {
            ...
            this.state.activeDrag = { type: type + '-line', element: canvas, pointId: null };
             ...
             // Allow clicking on the background of sliders/HS pane
    ...
    findHitPointSlider(x, y, type) {
        const activePoint = this.getActivePoint();
        const canvas = this.elements.interactiveCanvases[type];
        const lineY = type === 'brightness' ? (1 - activePoint.brightness) * canvas.height : (1 - activePoint.alpha) * canvas.height;
        let hitLine = Math.abs(y - lineY) <= 8;
        let hitPoint = null;
    ...
    handleClick(x, y, type, hitPoint) {
        if (hitPoint) {
            this.state.activePointId = hitPoint.id;
        } else if (type === 'hs') {
            this.createNewPointHS(x, y);
        } else if (type === 'brightness' || type === 'alpha') {
            // This allows clicking on the slider track to set the value
            const activePoint = this.getActivePoint();
            const canvas = this.elements.interactiveCanvases[type];
            const value = Math.max(0, Math.min(1, 1 - (y / canvas.height)));
            activePoint[type] = value;
            if (type === 'brightness') {
                this.constrainPointToValidArea(activePoint);
            }
        }
        this.drawAll();
    }
    ...
    createNewPointHS(x, y) {
        ...
        const av = (y - this.state.transform.offsetY) / this.state.transform.scale;
        const activePoint = this.getActivePoint();
        const {r, g, b} = this.abstractToRGB(au, av, activePoint.brightness);
        if (this.isValidColor(r, g, b)) {
            const newPoint = {
                id: Date.now(),
                hsPos: {u: au, v: av},
                originalHsPos: {u: au, v: av},
                brightness: activePoint.brightness,
                alpha: activePoint.alpha,
            };
            this.state.points.push(newPoint);
            this.state.activePointId = newPoint.id;
            this.drawAll();
        }
    }
    ...
    handleLineDrag(y, canvasHeight, dragType) {
        ...
        const property = dragType.startsWith('brightness') ? 'brightness' : 'alpha';
        const activePoint = this.getActivePoint();
        activePoint[property] = targetValue;
